        ...
        //  editRecord.setAttribute('src', 'images/edit.png');
          ...
          deleteRecord.onclick = function (){
           ...
           editRecord.onclick = function() {
               ...
               // userKey = getSessionPassword();
                $("#myEditModal").modal();
              // generateEditForm(userName,userPassword,userWebsite,userHint,userDate,userKey,uniqueKey);
...
function deleteRecord(userTable,userId,db){
...
function getEditDData(previousData,editedData,userKey,tableName){
 ...
 var key = null;
  var path = "";
 var userDataProperty=["userid","userpassword","userwebsite","userhint","date","userdate"];
...
console.log(editedData);
 ...
 console.log(previousData.userdate )
                    //remoteStorage.bicnSystCorp.removeUserData(previousData.userdate);
                    ...
                    deleteRecord(tableName,previousData.userdate,db);
...
function generateEditForm(userName,userPassword,userWebsite,userHint,userDate,userKey,uniqueKey){
    ...
    var popupTextBoxId = ["userid","userpassword","userwebsite","userhint"]
    var userDataRecord = [];

    var propertyArray = ["userid","userpassword","userwebsite","userhint","date","userdate"]
   ...
   userHintLabel =  document.createElement('Label');
    ...
    editUserHint.value = userHint;
     ...
     save.onclick = function(){
        ...
        console.log(getData(popupTextBoxId))
     //var obj = getUserDataObject(previousUserData,propertyArray);
...
selectChoice.onclick=function(){
   ...
   break;
                  ...
                  cancel.style.display ="block";
                   ...
                   search.style.marginLeft = 200 +"px";
                   search.style.marginTop = 20+"px";

                   cancel.style.marginLeft = 50 +"px";
                   cancel.style.marginTop = -40+"px";



                   from.style.marginTop = 20 +"px";
                   from.style.marginLeft = 20 +"px";

                   to.style.marginTop = 20 +"px";
...
function enterUserRecords(){
  ...
  userData[rowCount] = userDate;
   ...
   console.log(userData);
          ...
          console.log("remote"+obj.userdate);
...
function getDataNormalUser(){
    ...
    var decryptedUserData = [];
      ...
      var placeHolder =["fullname","userid","birthdate","email","mobile"];

    var userName = sessionStorage.getItem('recordKey') ;
...
function editUserInformation(previous,userKey,placeHolder,textBoxImage){
   ...
   // editDiv  = createIconButton( editDiv,'images/line2.png',"55px","20px");
    ...
    userData = getData(placeHolder);
     ...
     console.log(userData[3] + userKey);
...
function editPassword(){
   ...
   cancel = createImageButton('cancel',"50px","40px");
    ...
    save.onclick =  function(){
         ...
         var   fetchPassword = generateHashKey(userData[0]);
         ...
         if(checkPassword(oldPassword,fetchPassword))
         {
             ...
             if(checkPassword(userData[1],userData[2])){
                 ...
                 applyNewPasswordToUserInfo(userData[1],oldPassword);
                ...
                applyNewPasswordToUserData(userData[1],oldPassword);
...
function applyNewPasswordToUserInfo(newPassword,oldPassword){
         ...
         var placeHolder = []
          ...
          var Key  = "";
          var userDataProperty=["fullname","userid","birthdate","email","mobile"];
    ...
    db.executeSql('Select * from userinfo').then(function(records){
      ...
      for(items = 0 ;items < records.length ; items++){
            ...
            if(records[items].key == recordkey ){
                 ...
                 decryptedDa = decryptUserAllData(records[items],userDataProperty, userEnteredKey );


                  console.log(decryptedDa[1] + newUserPassword);

                 key = generateHashKey(decryptedDa[1] + newUserPassword);
                 ...
                 var editedData = getUserDataInObject(userDataProperty,decryptedDa,newUserPassword);


                  editedData["key"] = key;

                  sessionStorage.setItem('recordKey',key);

                    deleteRecord('userinfo',records[items].key,db);

                    storedUserRecords(editedData)
...
function applyNewPasswordToUserData(newPassword,oldPassword){
      ...
      var items = 0;
      var  userTextBoxId = ["userinfo","userwebsite","userhint","date","userdate"];
  var  userObjectId = ["username","userpassword","userwebsite","userhint","date","userdate"];
      var decryptedUserData = {};
     var  encryptUserData = {}  ;
...
console.log("new = "+newPassword);
    ...
    for(items = 0 ;items < records.length ; items++){
        ...
        decryptedData = decryptUserData(records[items],userTextBoxId,oldPassword);


       
       console.log(decryptedData);

      encryptUserData = createSensitiveEncryptedObject(decryptedData, userObjectId ,newPassword)


   console.log(encryptUserData);

         storeUserData(encryptUserData,'user',db);
...
function storeUserData(newRecord,tableName,db){


    addRecord(tableName,newRecord,db)
  ...
  var encryptedObj ={};
    ...
    console.log( encryptedData)
        ...
        if(validateSensitiveData(userArray[items])){
            ...
            encryptedObj[userArray[items]] = decryptData(encryptedData,userKey);
...
(function(){
  ...
  localStorage.setItem("localHostUrl", document.URL);
    localStorage.setItem("webUrl","http://localhost/html/sampledb/index.html");
...
if(results.length){
      ...
      for(numberOfItem = 0 ; numberOfItem < results.length ; numberOfItem++) {
                  ...
                  if(oldKey == results[numberOfItem].key){
                      ...
                      deleteRecord('userinfo',oldKey,db)
